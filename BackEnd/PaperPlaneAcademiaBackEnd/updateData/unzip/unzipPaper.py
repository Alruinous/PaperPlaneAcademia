...
# 将遍历到的 .gz 文件路径保存到 processed_papers_gz.txt
...
# 监控目录中的新 .gz 文件并解压
def monitor_new_files(directory, interval=10):
...
def start_monitoring(local_dir):
    processed_file = "/root/openalex-snapshot/data/processed_papers_gz.txt"  # 存储已处理 .gz 文件路径的文件
    ...
    monitor_new_files(local_dir)

# 示例调用
local_dir = "/root/openalex-snapshot/data/works"  # 本地文件夹路径
